...
if (!String.prototype.stripNonGermanDiacritics) {
  String.prototype.stripNonGermanDiacritics = function () {
    const umlautMap = {
      ä: '__ae__',
      ö: '__oe__',
      ü: '__ue__',
    ...
    // Protect German umlauts
    for (const char in umlautMap) {
      const placeholder = umlautMap[char];
      str = str.replace(new RegExp(char, 'g'), placeholder);
    }

    // Strip non-German diacritics
    str = str
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .normalize('NFC');
    ...
    for (const char in umlautMap) {
      const placeholder = umlautMap[char];
      str = str.replace(new RegExp(placeholder, 'g'), char);
    }
